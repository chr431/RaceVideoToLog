...
从 config.py 分离：本文件是"管线 + 共享"域的单一事实源，供
segment_flow / ocr_native / ocr_trt / segmentation / seg_correction /
video_utils / hybrid_decode 直接引用。config.py 通过
`from engine_config import *` 聚合再导出（GUI 与 `import config` 兼容），
GUI 专属常量（颜色/窗口/图表）留在 config.py。

第三步拆分时：engine_config.py + 管线模块整体迁出为独立仓库即可。
"""
