...
| video_ocr_engine | 0.7.x | git submodule chr431/video_ocr_engine | 解码+OCR 识别链引擎（FieldExtractor，零速度语义）；经 `third_party/video_ocr_engine`（sys.path bootstrap / .pth）提供引擎顶层模块与 `video_ocr_engine/` 包；OCR 模型资产随引擎仓库（PP-OCRv6_small + 字符表） |
