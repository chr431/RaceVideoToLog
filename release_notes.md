...
- 最终检查对话框的段预览改为解码**原始 RGB 帧**（生产管线仍用
  gray 输出保证性能；预览按需另开 CPU RGB reader，不再黑屏）；同时修复
  了初始预览被放大裁边的问题。对话框打开后后台**批量缓存全部代表帧
  RGB**（测试视频合计约 169MB，单视频最大 ~87MB；test5 全量 ~6.7s、
  test6 ~73s），点击任意未缓存段会把它立刻提到缓存队列最前，未就绪
  期间先显示灰度占位——长距离跳转不再卡界面，就绪后自动升级为彩色
