...
## v2.17.4（2026-09-19）— 引擎 0.14.1 对齐：shim 迁移 + decord 0.8.4 + 陈旧符号修正

> 本节面向使用者：只讲你能直接感知到的变化。技术细节见下方各节。

### 🎯 对你意味着什么

- **解码器升级到 decord 0.8.4**（自 0.8.2）：修了一个**多卡机器上跨设备
  拷贝静默产出错误数据**的符号名缺陷；混合解码的工作线程崩溃不再是无提示
  的进程退出，而是可诊断的报错。混合解码本身也更快（引擎侧实测）
- **引擎升级到 0.14.1**：引擎删除了六个历史遗留的兼容模块
  （`engine_config` / `ocr_native` / `video_utils` 等），本应用已同步迁移到
  新导入路径——**你无需任何操作**，但若你有自己的脚本 `import engine_config`，
  请改为 `from video_ocr_engine.config import constants`
- **修好了本应用自身的 17 处陈旧引用**：这些引用指向引擎早年改名的函数，
  此前让部分内部测试实际未运行（表面"通过"）。修好后可运行测试从
  81 增至 105 项（**全部通过**），对使用者意味着更可靠的分段/预处理行为

### 🔧 技术细节

- 引擎 shim 迁移：生产/测试/工具共 33 文件 59 行改包内路径
  （`video_ocr_engine.config.constants` 等六个新址）
- 陈旧符号修正：`_preprocess_standard` → `preprocess_standard`
  （引擎 C-32 轮已改名并迁至 `domain.segmentation`）；旧 `video_utils`
  兼容模块的"模块同一性"曾掩盖这些 ImportError
- 测试适配：`test_thread_budget` 的 env 设置改为**构造前**（引擎 0.14 起
  配置在构造期冻结，这是设计而非缺陷）
- release 流程修正：workflow 内两处引用已删引擎模块（否则发布即失败）

### 验证

本机全量测试 **105 passed / 1 skipped**（含 GUI 测试）；端到端实测
（hybrid 解码 + 3000 帧 ROI）产出 3007 段 CSV。
...
- **新增应用图标**：exe 文件图标、任务栏与窗口标题栏图标（此前 exe 使用
  ...
  横幅与生产流水线图。
- **GPU OCR 不再需要安装 CUDA Toolkit**：CUDA 运行时由最新 NVIDIA 驱动
  自带，TensorRT 用户只需将 TensorRT 11.x 的 `bin` 目录加入 PATH
  （NVDEC 视频解码本就只需驱动）。
...
- **OCR 输入 pad 宽度默认回退 224**（引擎 0.9.0 改为 160）：生产漏斗口径
  （分段代表帧 OCR + 检测/纠正链）实测 pad 160 使原始误读 150 → 190
  （test5 7→26、test6 17→32）；回退 224 后恢复 0.7.0 时代水平（150，
  最终错误 0），且 auto 路径墙钟零差（test5/test6 3000 帧 4.1/2.8s
  持平）。GUI 的 pad 宽度仍可调（160-320）。
- **CPU 解码/混合后端默认恢复完整去块滤波**（opt-out 引擎 0.9.0 的
  `DECORD_SKIP_LOOP_FILTER=all`）：滤波关在 CPU 解码路径实测段界漂移
  （+60 段）、检出率 97.9%、最终错误 0→7（test 3 / test2 4）——违反
  最终错误数硬门禁。代价是 CPU 解码 HEVC 场景放弃约 -12% 墙钟收益；
  生产默认 auto（NVDEC）本就不受该开关影响。如需引擎默认速度，可
  预先设置环境变量 `DECORD_SKIP_LOOP_FILTER=all`。

### 🔧 其他

- 本地/发布 decord fork 升级 0.7.14（`DECORD_SKIP_LOOP_FILTER` 透传
  需 ≥0.7.13）。
- 勘误：v2.17.0 笔记"原始误读 149→190 系关去块滤波"为错误归因——
  实测两个来源相互独立：pad 160（本版回退）与滤波关（本版 opt-out）。
...
- auto 与 hybrid 全量漏斗均 **最终错误 0**（5 视频 14533 段）：
  原始误读 149 → 190（引擎 0.9.0 OCR 输入宽度自适应裁切/pad 下限等
  预处理变化的账面波动），检出/纠正 **190/190 = 100%**、误改 0，
  与基线门禁一致无回归。
  （勘误 2026-08-29：初版笔记把该波动归因于关去块滤波，不实——
  本地 decord 0.7.11 下该 env 无效；且 auto 走 NVDEC 本就不受影响。）
...
- 新增**第二遍孤立尖峰检测**：在第一遍检测纠错的基础上，补抓
  "平台上的 2-off 单帧误读"（如 103 平台上读成 101 这类此前漏掉的
  小偏差误读），测试集最终错误 **11 → 0**（test 3→0 / test2 8→0，
  全部视频零错误、零误改）
- **test2 真值表更新为人工复核版本**：逐帧复核修正了旧真值表在
  4 帧上的系统性偏差（±2 km/h），并同步更新了该视频的 ROI 与参数
  （更贴近实际画面）；旧真值表导致的部分"漏纠"实为真值表本身的
  标注错误，现已修正
- 对 2-off 漏纠的结论修正：孤立 2-off 尖峰（值在邻域不重复）可修；
  仅"truth 瞬时跳变 / 值在邻域重复 / 贴合一侧"的类型才是不可区分
  的信息论极限

### 🔧 其他

- 无用户可见的界面/参数变化；内部参数见 CLAUDE.md（SEG_SPIKE_*）
...
- 后处理新增“一致性孤岛”保护：连续**近似相同**值（允许 ±2 km/h 内小波动）
  的累计帧数过少、且明显脱离局部曲线时，不再获得 HIGH_TRUST / DP 锚定
  （防止两个误读互相撑腰，也覆盖 127,128 这类轻微波动孤岛；完全相同 run
  需 ≥5 帧才可信，4 帧连续 127 这类短促平坦孤岛仍会被封顶）
