...
从 gui.py 提取的内聚单元：坐标换算、拖拽状态、节流重绘、
ROI 框绘制全部内聚于此，主窗口只负责提供帧数据和接收 ROI 变化。
"""
...
class PreviewWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._label.setStyleSheet(
            f"background-color: {config.PREVIEW_BG}; border-radius: 6px;")
    ...
    def _redraw(self) -> None:
        ...
        if self._roi is not None:
            ...
            painter.setPen(QPen(QColor(config.ROI_BOX_COLOR),
                                max(2, int(scale * 2))))
            painter.drawRect(l, t, r - l, b - t)
            painter.end()

        result = QPixmap(pw, ph)
        result.fill(QColor(config.CANVAS_FILL))
        rp = QPainter(result)
