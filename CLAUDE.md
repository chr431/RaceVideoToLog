...
| 视频 | 编码 | 单 auto | 默认双 2片 | 默认双 8片 | CPU+TRT×2 2片 |
|---|---|---:|---:|---:|---:|
| test | HEVC | 2.09s | 2.19s | 1.99s | 2.10s |
| test2 | h264 | 1.54s | 1.81s | 2.01s | 2.26s |
| test3 | h264 | 1.75s | **1.45s** | 1.93s | **1.62s** |
| test5 | h264 | 1.84s | **1.71s** | 1.93s | **1.65s** |
| test6 | AV1 | **1.61s** | 2.29s | 2.70s | 2.98s |

### 结论（针对“分片少 → GPU+TRT 闲置”的担忧）

- 动态切片确实把更多片交给快路径：test6 dual8 中 GPU+TRT 处理 5 片、
  CPU+ONNX 只处理 3 片；test dual8 中 GPU 处理 6 片、CPU 2 片。
- **但快路径闲置不能被切片数根治**：只要 CPU 解码/OCR 路径是瓶颈
  （AV1 尤其严重），GPU 路径做完自己的片后仍会等待 CPU 路径；
  分片越多只是让等待比例下降。
- 本机 Race 测试集上，**生产默认维持单 auto（GPU+TRT）**：它跨编码最稳。
  双流水线只在部分 h264 片（test3/test5）有 7~17% 收益，AV1 全部变慢
  （test6 +42~87%）。
- 两条 CPU+TRT 在 h264 可比默认双略快，但 AV1 CPU 软解是灾难，不能作为
  通用默认。
- 若要暴露给用户，必须保持默认关、用户显式开启，且建议默认切片数保持
  4（2~4 在本机多数场景最优，8 片在部分视频因寻址/边界开销反而更慢）。
