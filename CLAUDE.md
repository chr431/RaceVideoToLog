...
- **0.9.0 同步适配（2026-08-29）**：
  ...
  - **OCR 输入 pad 下限应用侧回退 224**（引擎 P0-5 改 160）：生产漏斗
    口径实测 pad 160 使原始误读 150→190（test5 7→26、test6 17→32），
    224 恢复 0.7.0 水平且 auto 墙钟零差（test5/test6 3000 帧 4.1/2.8s
    持平）——`config.py` 覆写 `engine_config.DEFAULT_FILL_WIDTH=224`
    （GUI 仍可调 160-320）。教训：引擎的"逐帧全等准确率"指标与
    本应用的"分段代表帧 + 纠错链"指标可给出相反结论，以本应用
    漏斗为准（2026-08-29 单变量 A/B，OCR_PAD_SMALL）。
...
- **0.7.0 公共 API 清理（破坏性）**：引擎移除 `fps` 构造参数（此前即被
  ...
  属性轨（`frames` 除外，保留）与 `prepare_review_rgb/timing_flat`；
  `video_utils` 移除 `open_decord_vr/VideoMetadata/format_duration/
  rss_mb/sum_nbytes`。
- **GPU 零拷贝管线默认转正**：`auto`（NVDEC+TRT）生产路线默认走
  分段/校准/合并/OCR 全程 GPU（`rep_crop_format` 支持 yuv/gray 代表帧）；
  `hybrid`/`cpu` 仍走宿主管线。
- **Race 侧适配**：段级公共属性（`frames/segment_frames/ocr_values/
...
- **用编码码流（参考/差值/帧级包大小）判断 ROI 是否变化、替代"解码→灰→二值→XOR"
  分段（用户提案，2026-08-20 探针量化后封板）**：设想"不解码、直接看码流的帧间
  参考/差值信息判断两帧 ROI 是否相同"。判定的前提是"读该信息"——取 ROI 局部逐块
  skip/残差必须做熵解码（CABAC/CAVLC/AV1 符号层），成本≈解码本身；故先用**最廉价
  码流信号**（ffprobe 帧级 `pkt_size/pict_type`，只解封装不重建像素）对"解码后 ROI
  XOR"真值做相关性探针（`tools/archive/_probe_stream_seg.py --video/--roi/--frames`）：
  - 移动背景（text_test AV1 字幕条 / test5 h264 赛车镜头）：分离度(变化帧>未变帧
    P90 的比例) **0.126 / 0.417**、预测精度 **0.126 / 0.417**、召回 **0.268 / 0.122**
    —— 整帧编码代价被全局运动淹没，码流字节数**无法预示 ROI 变化**（真实字幕场景
    主流即移动背景）。
  - 静态背景（test.mp4 HEVC 速度数字）：变化/未变中位包大小 37×、分离度 0.665、
    精度 0.665，但**召回仅 0.269** —— "包大小跳变"会漏掉 73% 的真实 ROI 变化。
  - 结论：① 移动背景下该信号不可用；② 静态背景下也只是弱辅助（召回差），且要提升到
    可用的"ROI 局部 skip 块占比"必须熵解码/fork 解码器（HF 成本=解码本身，AV1 尤甚），
    移动背景仍受全局运动干扰。**"不解码判断 ROI 变化"整体为死路**；现有 `sample_stride`
    分频采样已在不损失准确性的前提下拿到"少处理帧"的主体收益，勿再投入流级判断。
...
## 性能实验轮次：2026-08-21 引擎级双完整流水线（Race 实测）
...
| 视频 | 编码 | 单 auto | 默认双 2片 | 默认双 4片 | 默认双 8片 | CPU+TRT×2 2片 |
|---|---|---:|---:|---:|---:|---:|
| test | HEVC | 2.09s | 2.15s | **1.84s** | 1.91s | 2.10s |
| test2 | h264 | **1.50s** | 1.81s | 1.99s | 1.97s | 2.24s |
| test3 | h264 | 1.70s | **1.49s** | 1.72s | 1.95s | **1.60s** |
| test5 | h264 | 1.82s | **1.65s** | 1.94s | 1.83s | **1.65s** |
| test6 | AV1 | **1.60s** | 2.27s | 1.75s | 1.97s | 2.91s |

### 结论（针对“分片少 → GPU+TRT 闲置”的担忧）

- Plan B 后，多切片不再是“每片 join OCR”的硬屏障：
  - test HEVC 4 片 1.84s（优于单 2.09s）；
  - test6 AV1 4 片从 2.81s 降到 1.75s（但单 auto 1.60s 仍最优）；
  - 动态切片仍会把更多片交给快路径，例如 test6 dual4：GPU+TRT 3 片、
    CPU+ONNX 1 片；test dual4：GPU 3 片、CPU 1 片。
- **快路径闲置不可能被切片数根治**：只要 CPU 解码/OCR 路径是瓶颈，GPU 做完
  自己的片后仍会等待 CPU；Plan B 只是降低了等待比例，没有消灭等待。
- 本机 Race 测试集上，**生产默认仍应保持单 auto（GPU+TRT）**：跨编码最稳。
  双流水线只让部分 h264/HEVC 场景有收益，AV1 仍不是通用最优。
- 两条 CPU+TRT 在 h264 可比默认双略快，但 AV1 CPU 软解灾难，不能作为通用默认。
...
- 全量 Race 5 视频：
  ...
  - 各视频：test 1536→1482、test2 1223→1207、test3 1146→1114、
    test5 2533→2500、test6 8096→8002。
